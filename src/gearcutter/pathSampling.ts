...
import { pqpop, pqpush } from './utils/pq.js';
import { PolarCutSegment } from './types.js';

const BOTTOM_TOLERANCE = 0.00001;

export function normalizePolarCutPath(
  segments: PolarCutSegment[],
  dadt: number
): PolarCutSegment[] {
  ...
  const sampleAs: number[] = [];
  {
    ...
    for (const [sa, ea, curve, rot] of pq) {
      ...
      for (const a of curve.getDiscontinuityThetas(
        (sa - rot) * dadt,
        (ea - rot) * dadt
      )) {
  ...
  const bottomCuts: PolarCutSegment[] = [];
  const bottomCutStartSamples: number[] = [];
  const bottomCutEndSamples: number[] = [];
  let preva: number = -0.5;
  const activeCuts: PolarCutSegment[] = [];
  ...
  for (const cura of sampleAs) {
    ...
    if (prevCandidates.length) {
      const survivors =
        curMin == null ? [] : getBottomSegs(prevCandidates, dadt, cura, curMin);
      if (!survivors.length) {
...
function getBottomSegs(
  cuts: PolarCutSegment[],
  dadt: number,
  a: number,
  bottom: number
): PolarCutSegment[] {
  const ret: PolarCutSegment[] = [];
