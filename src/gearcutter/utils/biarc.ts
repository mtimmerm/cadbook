...
export function biArcSplit(
  ...
  const k =
    dmag /
    ((tx0 * txm + ty0 * tym + 1) * f0 + (tx1 * txm + ty1 * tym + 1) * f1);
  const a0 = k * f0;
 ...
 */
export function biArcApproximate(
  samples: PointAndTangent[],
  tolerance: number
): PointAndTangent[] {
...
function getApproxError(
  ...
  for (let i = fromPos + 1; i < toPos; ++i) {
    maxError = Math.max(
      maxError,
      getBiArcPointError(pt0, ptm, pt1, samples[i])
    );
  }
  return maxError;
}
