...
import { biArcApproximate, drawBiArc, PointAndTangent } from './utils/biarc.js';
import { searchForFloat } from './utils/floatBinarySearch.js';
import { CutCurve } from './types.js';
...
export class CircleCut implements CutCurve {
  ...
  private getPointAndTangentForT(
    t: number,
    tangentSign: number
  ...
  private getTForTheta(theta: number): number {
    if (
      this.reversal &&
      theta >= this.reversal.thetaStart &&
      theta <= this.reversal.thetaEnd
    ) {
  ...
  private getTForThetaInBase(theta: number): number {
    ...
    [tmin, tmax] = searchForFloat(
      tmin,
      tmax,
      (t) => this.getThetaForT(t) >= theta
    );
