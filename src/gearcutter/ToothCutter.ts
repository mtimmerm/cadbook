import { CircleCut } from './CircleCut.js';
import { ConstantRadiusCut } from './ConstantRadiusCut.js';
import { normalizePolarCutPath } from './pathSampling.js';
import { CutCurve, Pen, PolarCutSegment } from './types.js';
 ...
 */
export class ToothCutter implements Pen {
  ...
  drawToothPath(pen: Pen, doInitialMove: boolean): void {
    ...
    for (const seg of segments) {
      ...
      c.drawSegment(
        xpen,
        (sa - rot) * this.dadTooth,
        (ea - rot) * this.dadTooth,
        doInitialMove
      );
      doInitialMove = false;
    }
  ...
  arcTo(x: number, y: number, turn: number): void {
    ...
    const dirx = y0 - y;
    const diry = x - x0;
    const k = (this.dydTooth * diry) / (dirx * dirx + diry * diry);
    const dxdt = k * dirx;
    const dydt = k * diry;
    ...
    this.path.push([
      Math.min(thetaA, thetaB),
      Math.max(thetaA, thetaB),
      curve,
      0,
    ]);
  }
